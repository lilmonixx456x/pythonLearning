...
from .acl import extract_candidate_hijack_paths, find_writable_parent, get_identity_context, has_unquoted_space_path, is_registry_path_writable
...
ESCALATION_RISKS = {
    ...
    "cleartext_autologon_password",
    "service_binary_replacement_or_drop",
    "high_confidence_service_hijack",
    "service_dll_hijack",
    "service_imagepath_hijack",
    "task_binary_replacement_or_script_swap",
    "task_working_dir_hijack",
    "autorun_binary_swap",
    "path_hijack",
    "wmi_command_consumer_persistence",
    "wmi_script_consumer_persistence",
    "share_ntfs_write_overlap",
    "correlated_privileged_write_path",
    "share_backed_privileged_binary_path",
    "privileged_registry_write_surface",
    "privileged_service_registry_write",
    "privileged_task_file_write",
    "appcertdlls_injection_surface",
    "ifeo_monitor_process_surface",
    "lsa_provider_injection_surface",
}
...
def check_registry_write_surfaces(identity):
    findings = []
    paths = [
    ...
    for registry_path in paths:
        acl = is_registry_path_writable(registry_path, identity)
        if acl["writable"]:
            level = "critical" if registry_path.endswith("\\Services") or "Winlogon" in registry_path or "Lsa" in registry_path else "high"
            findings.append(make_finding("registry_acl", "Writable high-value registry path", level, f"{registry_path} | acl={acl['reason']}", "privileged_registry_write_surface", registry_path, confidence="confirmed", refs={"acl": acl}))
    return {"findings": findings, "meta": {"checked": paths}}
...
def check_service_registry_permissions(identity):
    findings = []
    data = powershell_json(
        "Get-CimInstance Win32_Service | Select-Object Name,StartName | ConvertTo-Json -Depth 3"
    )
    if isinstance(data, dict) and data.get("error"):
        return {"findings": [make_finding("service_registry_acl", "Service registry ACL inspection failed", "medium", data["error"], "coverage_gap", confidence="weak")], "meta": {}}
    checked = 0
    for item in data:
        start_name = str(item.get("StartName") or "").lower()
        if not any(token in start_name for token in ("localsystem", "localservice", "networkservice", "nt authority")):
            continue
        service_name = item.get("Name")
        registry_path = fr"HKLM:\SYSTEM\CurrentControlSet\Services\{service_name}"
        acl = is_registry_path_writable(registry_path, identity)
        checked += 1
        if acl["writable"]:
            findings.append(make_finding("service_registry_acl", "Privileged service registry key writable", "critical", f"{service_name} | {registry_path} | acl={acl['reason']}", "privileged_service_registry_write", registry_path, confidence="confirmed", refs={"service": service_name, "acl": acl}))
...
def check_task_files(identity):
    findings = []
    data = powershell_json(
        "Get-ScheduledTask | Select-Object TaskName,TaskPath,@{n='RunAs';e={$_.Principal.UserId}} | ConvertTo-Json -Depth 4"
    )
    if isinstance(data, dict) and data.get("error"):
        return {"findings": [make_finding("task_files", "Task file inspection failed", "medium", data["error"], "coverage_gap", confidence="weak")], "meta": {}}
    checked = 0
    for item in data:
        run_as = str(item.get("RunAs") or "").lower()
        if not any(token in run_as for token in ("system", "administrator", "service")):
            continue
        rel = f"{item.get('TaskPath', '').strip('\\')}"
        name = item.get("TaskName") or ""
        task_file = Path(r"C:\Windows\System32\Tasks")
        if rel:
            task_file = task_file / rel
        task_file = task_file / name
        checked += 1
        if task_file.exists():
            acl = find_writable_parent(str(task_file), identity)
            if acl["writable"]:
                findings.append(make_finding("task_files", "Privileged task file writable", "critical", f"{task_file} | acl={acl['reason']}", "privileged_task_file_write", str(task_file), confidence="confirmed", refs={"task": item, "acl": acl}))
    return {"findings": findings, "meta": {"checked": checked}}


def check_appcertdlls():
    findings = []
    script = (
        "$path='HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\AppCertDlls';"
        "if(Test-Path $path){Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | ForEach-Object {$_.PSObject.Properties | "
        "Where-Object {$_.Name -notmatch '^PS'} | ForEach-Object {[pscustomobject]@{Name=$_.Name;Value=$_.Value}}}} | ConvertTo-Json -Depth 3"
    )
    data = powershell_json(script)
    if isinstance(data, dict) and data.get("error"):
        return {"findings": [make_finding("appcertdlls", "AppCertDlls inspection failed", "medium", data["error"], "coverage_gap", confidence="weak")], "meta": {}}
    for item in data:
        findings.append(make_finding("appcertdlls", "AppCertDlls entry present", "critical", f"{item.get('Name')} => {item.get('Value')}", "appcertdlls_injection_surface", confidence="confirmed", refs=item))
    return {"findings": findings, "meta": {"count": len(data) if isinstance(data, list) else 0}}


def check_app_paths(identity):
    findings = []
    script = (
        "$root='HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths';"
        "if(Test-Path $root){Get-ChildItem $root -ErrorAction SilentlyContinue | ForEach-Object {"
        "  $p=Get-ItemProperty -Path $_.PSPath -ErrorAction SilentlyContinue;"
        "  [pscustomobject]@{Name=$_.PSChildName;Path=$p.'(default)';BasePath=$p.Path}"
        "}} | ConvertTo-Json -Depth 4"
    )
    data = powershell_json(script, timeout=90)
    if isinstance(data, dict) and data.get("error"):
        return {"findings": [make_finding("app_paths", "App Paths inspection failed", "medium", data["error"], "coverage_gap", confidence="weak")], "meta": {}}
    for item in data:
        for candidate in (expand_env_path(item.get("Path") or ""), expand_env_path(item.get("BasePath") or "")):
            if candidate:
                parent = first_existing_parent(candidate)
                if parent:
                    acl = find_writable_parent(parent, identity)
                    if acl["writable"]:
                        findings.append(make_finding("app_paths", "App Paths entry points to writable location", "high", f"{item.get('Name')} | {candidate} | acl={acl['reason']}", "app_paths_hijack_surface", candidate, confidence="confirmed", refs={"entry": item, "acl": acl}))
    return {"findings": findings, "meta": {"count": len(data) if isinstance(data, list) else 0}}


def check_ifeo_monitor_process():
    findings = []
    script = (
        "$ifeo='HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options';"
        "$silent='HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\SilentProcessExit';"
        "$out=@();"
        "if(Test-Path $ifeo){Get-ChildItem $ifeo -ErrorAction SilentlyContinue | ForEach-Object {"
        "  $g=(Get-ItemProperty -Path $_.PSPath -Name GlobalFlag -ErrorAction SilentlyContinue).GlobalFlag;"
        "  if($g){$out += [pscustomobject]@{Key='IFEO';Name=$_.PSChildName;GlobalFlag=$g}}"
        "}};"
        "if(Test-Path $silent){Get-ChildItem $silent -ErrorAction SilentlyContinue | ForEach-Object {"
        "  $m=(Get-ItemProperty -Path $_.PSPath -Name MonitorProcess -ErrorAction SilentlyContinue).MonitorProcess;"
        "  if($m){$out += [pscustomobject]@{Key='SilentProcessExit';Name=$_.PSChildName;MonitorProcess=$m}}"
        "}};"
        "$out | ConvertTo-Json -Depth 4"
    )
    data = powershell_json(script)
    if isinstance(data, dict) and data.get("error"):
        return {"findings": [make_finding("ifeo_silent", "IFEO/SilentProcessExit inspection failed", "medium", data["error"], "coverage_gap", confidence="weak")], "meta": {}}
    for item in data:
        if item.get("MonitorProcess"):
            findings.append(make_finding("ifeo_silent", "SilentProcessExit monitor configured", "critical", f"{item.get('Name')} => {item.get('MonitorProcess')}", "ifeo_monitor_process_surface", confidence="confirmed", refs=item))
        else:
            findings.append(make_finding("ifeo_silent", "IFEO GlobalFlag present", "high", f"{item.get('Name')} => GlobalFlag={item.get('GlobalFlag')}", "ifeo_monitor_process_surface", confidence="confirmed", refs=item))
    return {"findings": findings, "meta": {"count": len(data) if isinstance(data, list) else 0}}


def check_lsa_providers(identity):
    findings = []
    script = (
        "$path='HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Lsa';"
        "$props=Get-ItemProperty -Path $path -ErrorAction SilentlyContinue;"
        "[pscustomobject]@{"
        "AuthenticationPackages=$props.'Authentication Packages';"
        "SecurityPackages=$props.'Security Packages';"
        "NotificationPackages=$props.'Notification Packages'"
        "} | ConvertTo-Json -Depth 4"
    )
    data = powershell_json(script)
    if isinstance(data, dict) and data.get("error"):
        return {"findings": [make_finding("lsa_packages", "LSA package inspection failed", "medium", data["error"], "coverage_gap", confidence="weak")], "meta": {}}
    record = data[0] if isinstance(data, list) and data else {}
    for key in ("AuthenticationPackages", "SecurityPackages", "NotificationPackages"):
        values = record.get(key) or []
        if isinstance(values, str):
            values = [values]
        suspicious = [v for v in values if str(v).strip() and str(v).lower() not in ("msv1_0", "kerberos", "schannel", "wdigest", "tspkg", "pku2u", "cloudap", "negoexts")]
        if suspicious:
            findings.append(make_finding("lsa_packages", "Non-default LSA package/provider configured", "critical", f"{key} => {', '.join(map(str, suspicious))}", "lsa_provider_injection_surface", confidence="confirmed", refs={"key": key, "values": suspicious}))
    acl = is_registry_path_writable(r"HKLM:\SYSTEM\CurrentControlSet\Control\Lsa", identity)
    if acl["writable"]:
        findings.append(make_finding("lsa_packages", "LSA registry path writable", "critical", f"HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Lsa | acl={acl['reason']}", "privileged_registry_write_surface", r"HKLM:\SYSTEM\CurrentControlSet\Control\Lsa", confidence="confirmed", refs={"acl": acl}))
    return {"findings": findings, "meta": record}


def check_group_policy_scripts():
    findings = []
    domain = os.environ.get("USERDNSDOMAIN") or os.environ.get("USERDOMAIN") or ""
    if not domain:
        return {"findings": findings, "meta": {"domain": ""}}
    roots = [
        fr"\\{domain}\SYSVOL\{domain}\Policies",
        fr"\\{domain.lower()}\SYSVOL\{domain.lower()}\Policies",
    ]
    checked = 0
    for root in roots:
        path = Path(root)
        ...
        for base, _, files in os.walk(root):
            for name in files:
                checked += 1
                if checked > 4000:
                    return {"findings": findings, "meta": {"checked": checked, "roots": roots, "truncated": True}}
                lower = name.lower()
                if lower in ("scripts.ini", "psscripts.ini") or lower.endswith((".bat", ".cmd", ".ps1", ".vbs")):
                    full = str(Path(base) / name)
                    content = read_text_file(full, 12000)
                    findings.append(make_finding("gpo_scripts", "GPO script artifact present", "info", f"{full} | size={len(content)}", "gpo_script_surface", full, confidence="confirmed"))
    return {"findings": findings, "meta": {"checked": checked, "roots": roots}}
...
def build_escalation_assessment(results):
    confirmed = [item for item in results["findings"] if item.get("confidence") == "confirmed" and item.get("risk") in ESCALATION_RISKS]
    probable = [item for item in results["findings"] if item.get("risk") in ESCALATION_RISKS]
    classes = sorted({item["risk"] for item in confirmed})
    if confirmed:
        return {
            "possible": True,
            "confidence": "confirmed",
            "classes": classes,
            "summary": "Confirmed privilege-escalation exposure classes exist for a non-admin local user. Review and close the listed classes immediately.",
        }
    if probable:
        return {
            "possible": False,
            "confidence": "weak",
            "classes": sorted({item["risk"] for item in probable}),
            "summary": "No confirmed admin-level escalation path was proven by configuration correlation, but multiple exposure classes remain and need manual closure.",
        }
    return {
        "possible": False,
        "confidence": "confirmed",
        "classes": [],
        "summary": "No confirmed admin-level escalation path was identified from the current non-admin visibility.",
    }
...
def run_all_checks(target):
    ...
    modules = [
        ...
        ("registry_acl", lambda: check_registry_write_surfaces(identity)),
        ...
        ("service_registry_acl", lambda: check_service_registry_permissions(identity)),
        ("tasks", lambda: check_tasks(identity)),
        ("task_files", lambda: check_task_files(identity)),
        ...
        ("appcertdlls", check_appcertdlls),
        ("app_paths", lambda: check_app_paths(identity)),
        ("ifeo_silent", check_ifeo_monitor_process),
        ("lsa_packages", lambda: check_lsa_providers(identity)),
        ("gpo_scripts", check_group_policy_scripts),
        ("files", None),
    ...
    results["escalation_assessment"] = build_escalation_assessment(results)
    return results
